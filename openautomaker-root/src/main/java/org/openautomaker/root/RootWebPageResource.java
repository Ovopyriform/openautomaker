...
public class RootWebPageResource
{
	...
	@Inject
	public RootWebPageResource(PrinterRegistry printerRegistry) {
	...
	{
		printerRegistry.setServerName(serverName);
		// If this server is connected to just one RBX10 printer, then
		// assume it is a RoboxPro. Name the printer to be the same as the
		// server.
		if (printerRegistry.getRemotePrinters().size() == 1)
		{
			printerRegistry.getRemotePrinters().forEach((k,v) ->
			{
				try
				{
