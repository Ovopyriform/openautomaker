...
public class StatusData
{
	...
	public void updateFromPrinterData(String printerID)
	{
		...
		PrinterColourMap colourMap = RootServices.getPrinterColourMap();
		Color displayColour = colourMap.printerToDisplayColour(printer.getPrinterIdentity().printerColourProperty().get());

		printerWebColourString = "#" + colourToHex(displayColour);
		...
		case UNLOADING_FILAMENT_E:
			printerStatusString = RootServices.getI18N().t(printer.busyStatusProperty().get().getI18nString());
		...
		if (!statusProcessed)
		{
			...
			case PAUSE_PENDING:
			case RESUME_PENDING:
				printerStatusString = RootServices.getI18N().t(printer.pauseStatusProperty().get().getI18nString());
		...
		if (!statusProcessed)
		{
			...
			case PURGING_HEAD:
			case REMOVING_HEAD:
				printerStatusString = printer.printerStatusProperty().get().getKey();
				printerStatusEnumValue = printer.printerStatusProperty().get().name();
				statusProcessed = true;
				break;
			case RUNNING_MACRO_FILE:
				printerStatusString = printer.getPrintEngine().macroBeingRun.get().getMacroFileName();
				printerStatusEnumValue = printer.printerStatusProperty().get().name();
				statusProcessed = true;
				break;
			default:
		...
		for (int extruderNumber = 0; extruderNumber < numberOfExtruders; extruderNumber++)
		{
			...
			if (printer.effectiveFilamentsProperty().get(extruderNumber) != FilamentContainer.UNKNOWN_FILAMENT)
			{
				...
				webColour = "#" + colourToHex(filament.getDisplayColourProperty().get());
